...
export class ShiftService {
  ...
  async findAll(
    ...
    const { data, total } = await this.shiftRepo.findAllByOrg(
      user.organizationId,
      pagination,
    );
    return {
  ...
  async update(
    ...
    return (await this.shiftRepo.update(id, {
      ...dto,
      modified_by: user.userId,
    }))!;
  }
  ...
  async changeStatus(
    ...
    if (dto.status === Status.INACTIVE) {
      const activeRuleCount = await this.ruleRepo.countActiveByShiftId(
        shift.id,
      );
    ...
    this.logger.log(
      `Shift ${id} status changed to ${dto.status} by ${user.userId}`,
    );
  }
