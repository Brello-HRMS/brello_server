...
import { OrganizationSubscription, SubscriptionStatus } from '../../plan/entities/organization-subscription.entity';
...
export class PermissionResolverService {
    ...
    private async getActivePlanId(organizationId: string): Promise<string | null> {
        const now = new Date();
    ...
    private async applyPlanRestrictions(
        ...
        const enabledModuleIds = new Set(enabledPlanModules.map((pm) => pm.module_id));
        ...
        for (const [moduleId, roleActions] of rolePermissions.entries()) {
            ...
            if (!planAllowed || planAllowed.size === 0) {
                ...
                effectiveActions = new Set([...roleActions].filter((a) => planAllowed.has(a)));
            }
    ...
    private async getActionNames(actionIds: string[]): Promise<Map<string, string>> {
